...
func (s *Server) handle(w mdns.ResponseWriter, r *mdns.Msg) {
    if len(r.Question) == 0 { _ = w.WriteMsg(new(mdns.Msg)); return }
    q := r.Question[0]
    ...
    if err != nil { s.writeServFail(w, r); return }
    _ = w.WriteMsg(resp)
}
...
func (s *Server) match(name string, suffixes []string) bool {
    for _, sfx := range suffixes {
        ...
        if sfx == "" { continue }
        if strings.HasSuffix(name, strings.TrimPrefix(sfx, ".")) { return true }
    }
    return false
}
...
func (s *Server) forward(ctx context.Context, req *mdns.Msg, ups []Upstream) (*mdns.Msg, error) {
    ...
    for _, up := range ups {
        ...
        if err == nil && resp != nil { return resp, nil }
        lastErr = err
    }
    if lastErr == nil { lastErr = errors.New("no upstream") }
    return nil, lastErr
}

func netProto(addr string) string {
    if strings.HasPrefix(addr, "tls://") || strings.HasPrefix(addr, "https://") { return "tcp" }
    return "udp"
}
