...
func (m *SyncManager) fetch(ctx context.Context, url string) ([]byte, error) {
    ...
    if resp.StatusCode >= 300 { return nil, errors.New(resp.Status) }
    return io.ReadAll(resp.Body)
}

func setToSlice(set map[string]struct{}) []string {
    out := make([]string, 0, len(set))
    for d := range set { out = append(out, d) }
    sort.Strings(out)
    return out
}
...
func parseDomainsFromChinaList(s string) map[string]struct{} {
    ...
    for _, line := range strings.Split(s, "\n") {
        ...
        if strings.HasPrefix(line, "server=/") {
            ...
            if d, ok := normalizeDomain(seg); ok { out[d] = struct{}{} }
            continue
        }
        if d, ok := normalizeDomain(line); ok { out[d] = struct{}{} }
    }
    return out
}
...
func parseDomainsGeneric(s string) map[string]struct{} {
    ...
    for _, line := range strings.Split(s, "\n") {
        ...
        if len(fields) >= 2 {
            if d, ok := normalizeDomain(fields[1]); ok { out[d] = struct{}{} }
            continue
        }
        // address=/domain/
